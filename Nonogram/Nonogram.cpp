...
void printOutTable(unsigned** mainGrid, unsigned** rowClues, unsigned** colClues, size_t gridSize, size_t maximumRowClues, size_t maximumColClues)
{
	...
	{
		// the reason we used the ROW_CLUE_SCANNER_BUFFER that we calculated is to jump over the columns without any elements in them
...
void closePointerSpace(unsigned** matrix, size_t gridSize) {
	// this method is used for removing matrices that are not needed anymore
...
void readAndInitializePuzzleFromFile(char* fileName, size_t level, unsigned** puzzle, unsigned** rowClues, unsigned** colClues, size_t& maxRowBuffer, size_t& maxColBuffer) {
	...
	const size_t STRING_SIZE = gridSize * gridSize + 1;
	...
	std::ifstream reader;
	char* puzzleString = new char[STRING_SIZE]; // the size is set that way because it needs to accomodate size^2 elements and the terminating zero - hence the + 1
...
int main()
{
	...
	do {
			...
			else
			{
				...
				if (puzzle[x - 1][y - 1] == 0 && playerGrid[x - 1][y - 1] == 1) {
					...
					if (numberOfAttempts == 0) {
						...
						closePointerSpace(puzzle, currentPuzzleDimension);
						closePointerSpace(rowClues, currentPuzzleDimension);
						closePointerSpace(colClues, currentPuzzleDimension);
						closePointerSpace(playerGrid, currentPuzzleDimension);
						return 0;
					}
					else std::cout << "Your clue was wrong! Please be more careful next time! Your attempts are now " << numberOfAttempts << std::endl;
				}
			}
		...
		if (roundPicker == 'e') {
			saveUserInfo(fileOpenerAndWriter, userFile, password, level, score, puzzleName, playerGrid, currentPuzzleDimension);
			closePointerSpace(puzzle, currentPuzzleDimension - 1);
			closePointerSpace(rowClues, currentPuzzleDimension - 1);
			closePointerSpace(colClues, currentPuzzleDimension - 1);
			closePointerSpace(playerGrid, currentPuzzleDimension);
			return 0;
		}
